...
func Parse(args []string) (cfg *Config, printVersion bool, err error) {
	...
	upstreamUser := fs.String("upstream-user", "", "bitcoind auth user (health probe + fallback for unauth forwards)")
	upstreamPassword := fs.String("upstream-password", "", "bitcoind auth password (see --upstream-user)")
