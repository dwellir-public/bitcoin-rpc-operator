...
Licensed under BSD 3-clause (see LICENSE.bitcoind-monitor in this directory).
...
Minor modifications by Jakob Ersson for better compatibility with Juju charm use.
Synced with upstream main as of 2026-06.
"""
...
import riprova
from bitcoin.rpc import InWarmupError, JSONRPCError, Proxy
...
def rpc_client_factory():
    ...
    if use_conf:
        ...
        logger.info(f"Host: {BITCOIN_RPC_HOST}, port: {BITCOIN_RPC_PORT}, user: {BITCOIN_RPC_USER}")
...
def main():
    ...
    def refresh_app(*args, **kwargs):
        ...
        try:
            ...
            logger.exception("Refresh failed during retry.")
            exception_count(e)
        except JSONRPCError as e:
            logger.exception("Bitcoin RPC error refresh", exc_info=True)
            exception_count(e)
        except json.decoder.JSONDecodeError:
            logger.exception("RPC call did not return JSON. Bad credentials?")
            sys.exit(1)
